...
def preprocess():
    ...
    sel_feat = np.invert(x)
    train_data = train_data[:,sel_feat]
    validation_data = validation_data[:,sel_feat]
    test_data = test_data[:,sel_feat]
    
    selected_features = np.where(sel_feat)[0]
...
def nnObjFunction(params, *args):
    ...
    training_data = np.hstack((bias,training_data))

    hidden_lay = np.dot(training_data,w1.T)
    hidden_act_old = sigmoid(hidden_lay)

    hidden_act = np.hstack((bias,hidden_act_old))
    ...
    true_labels = np.zeros((train_size,n_class))


    training_label = training_label.astype(int)
    
    true_labels[np.arange(train_size),training_label]=1.0
...
def nnPredict(w1, w2, data):
    ...
    hidden_act = np.hstack((bias,hidden_act))
...
train = str(100 * np.mean((predicted_label == train_label).astype(float)))
...
valid = str(100 * np.mean((predicted_label == validation_label).astype(float)))
...
print("Total time elapsed: %s"%(time_elaspsed))


test = str(100 * np.mean((predicted_label == test_label).astype(float)))


line = str(lambdaval)+"  "+train+"  "+test+"  "+valid+"  "+str(time_elaspsed)+"\n"

with open("reg_params.txt", "a+") as myfile:
    myfile.write(line)
